...
// EventPublisher wraps the commons Publisher to work with authentication events
...
// PublishEvent publishes an authentication event
func (ep *EventPublisher) PublishEvent(ctx context.Context, event Event) error {
	return ep.PublishWithRetry(ctx, event, 3)
}

// PublishWithRetry publishes an event with retry logic
func (ep *EventPublisher) PublishWithRetry(ctx context.Context, event Event, maxRetries int) error {
	...
	ep.logger.Info("event published successfully",
		"event_type", event.GetType(),
		"event_id", event.GetID(),
		"routing_key", routingKey)
